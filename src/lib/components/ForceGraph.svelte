		...
		simulation = forceSimulation(nodes)
			...
			.force(
				...
				forceCollide().radius((d) =>
					d.depth == 0 ? 100 : d.depth == 1 ? 24 : d.depth == 2 ? 12 : 4
				)
			)
...
<svg {width} {height}>
	...
	{#each nodes as point}
		{#if activePageData && point.id == activePageData.title}
			<circle r={10} stroke={'black'} stroke-width={3} cx={point.x} cy={point.y} />
		{/if}
		{#if point.depth || point.depth == 0}
			<a href={point.depth == 3 ? point.id : 'tag/' + slugify(point.id)}>
				<circle
					class="node"
					r={point.depth == 0 ? 12 : point.depth == 1 ? 7 : point.depth == 2 ? 5 : 4}
	...
	{#each nodes as point}
		{#if point.depth || point.depth == 0}
			{#if point.depth == 1}
				<text class='texthalo' x={point.x} y={point.y} text-anchor={'middle'} fill={colorScale[point.id]}>{point.id}</text>
			{/if}
		{/if}
	{/each}
</svg>
